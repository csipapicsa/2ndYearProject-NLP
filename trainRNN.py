...
def trainRNN(data_sets, y_train, y_test, early_stop_patience=2, filename="train", maxlen=40):
    ### INIT RESULTS
    ...
    for data_set in data_sets:
        ...
        Test = tokenizer.texts_to_sequences(data_set[2])
            # Sequencer 
        X_train_p = pp.sequence_pad(Train, maxlen=maxlen) # there are several attributes which can be defined, basic = first 50 words 
        X_test_p = pp.sequence_pad(Test,maxlen=maxlen)

            # TRAIN
        #print("shapes: ", X_train_p.shape, X_test_p.shape)
        history, model = nn.RNN_train(X_train_p, y_train, X_test_p, y_test, tokenizer, 
                                      maxlen=maxlen, early_stop_patience=early_stop_patience, batch_size=batch_size)
        ### LOGGING INIT - RNN
        ...
        ratio_train, ratio_test = f.lengths_catch(data_set[1], data_set[2], length_of_sentence=maxlen)
